        ...
        while self.running:
            ...
            if 'username' in decodedMessage and 'password' in decodedMessage:
                ...
                if self.users[payer]['balance'] < amount_transferred:
                    ...
                    response['tx_id'] = self.users[payer]['tx_id']
            ...
            elif 'action' in decodedMessage and decodedMessage['action'] == 'fetch_transactions':
                self.signal.emit(f'Received Fetch & Display Request from user {username}')
                # Extract username from the message
                username = decodedMessage.get('username', None)
                if username is None:
                   # If username is not provided, send an error response
                   response = {'error': 'Username not provided'}
                   self.signal.emit('Error: Username not provided')
                   self.serverSocket.sendto(json.dumps(response).encode(), clientAddress)
                else:
                   # Check if the user exists
                   if username in self.users:
                    # Prepare response with user's balance and transactions
                        response = {
                           'balance': self.users[username]['balance'],
                            'txs': self.users[username]['txs']
                                   }
                        self.signal.emit(f'Sending transactions and balance for user {username}')
                   else:
                        # If user doesn't exist, send an error response
                         response = {'error': 'User not found'}
                         self.signal.emit(f'Error: User {username} not found')
